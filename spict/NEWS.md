...
* On load, `spict` checks if the currently available TMB version is the same
  ...
  mismatch.
* The harvest control rule (HCR) labeled "ices" in the `manage()` function was
  updated to be in line with the HCR recommended to be used by ICES in 2025.
* The management section of the guidelines vignette was updated to be in line
  with current recommendations by ICES (state 2025).
...
* `intermediatePeriodCatchList` is now used by the manage function, previously was
   ignored
* The retrospective peels generated by the `retro()` function did not behave as
