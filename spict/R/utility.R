...
write.aspic <- function(input, filename='spictout.a7inp'){
    inp <- check.inp(input)
    obstimes <- sort(unique(c(inp$timeC, inp$timeI[[1]])))
    nobs <- length(obstimes)
    dat <- cbind(obstimes, rep(-1, nobs), rep(-1, nobs))
    inds <- match(inp$timeI[[1]], obstimes)
    dat[inds, 2] <- inp$obsI[[1]]
    inds <- match(inp$timeC, obstimes)
    dat[inds, 3] <- inp$obsC
    ...
    for(i in 1:inp$nindex) cat(sprintf('q     %3.2E  1  %3.2E  %3.2E  %3.2E\n', exp(inp$ini$logq[i]), 1, 0.01*exp(inp$ini$logq[i]), 100*exp(inp$ini$logq[i])), file=filename, append=TRUE)
    cat('DATA\n', file=filename, append=TRUE)
    cat('"Combined-Fleet Index, Total Landings"\n', file=filename, append=TRUE)
    cat('CC\n', file=filename, append=TRUE)
    for(i in 1:nobs) cat(sprintf('  %4i    % 6.4E    % 6.4E\n', obstimes[i], dat[i, 2], dat[i, 3]), file=filename, append=TRUE)
    if(inp$nindex>1){
        for(I in 2:inp$nindex){
            cat(paste0('"Index',I-1,'"\n'), file=filename, append=TRUE)
            cat('I1\n', file=filename, append=TRUE)
            dat <- cbind(obstimes, rep(-1, nobs))
            inds <- match(inp$timeI[[I]], obstimes)
...
read.aspic.res <- function(filename){
    ...
    K <- 2*Bmsy
    r <- 2*Fmsy
    return(c(r=r, K=K, q=q, Fmsy=Fmsy, Bmsy=Bmsy, MSY=MSY))
}
