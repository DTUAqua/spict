...
#' @return Nothing.
#' @export
plotspict.biomass <- function(rep){
    inp <- rep$inp
    # Biomass plot
...
summaryspict <- function(rep, numdigits=4){
    ...
    if(rep$opt$convergence>0) cat('Model did not obtain proper convergence! Estimates and uncertainties are most likely invalid and should not be used.\n')
    cat(paste('Negative log likelihood: ', round(rep$opt$objective, numdigits), '\n', sep=''))
    cat('\nModel parameter estimates \n')
    sd <- sqrt(diag(rep$cov.fixed))
    resout <- cbind(estimate=round(exp(rep$par.fixed),numdigits), cilow=round(exp(rep$par.fixed-1.96*sd),numdigits), ciupp=round(exp(rep$par.fixed+1.96*sd),numdigits), est.in.log=round(rep$par.fixed,numdigits))
    rownames(resout) <- substr(names(rep$par.fixed),4,6)
