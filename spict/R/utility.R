...
#'
#' Fills in defalut values if missing.
...
# Check initial values of required model parameters
...
# Get the map used to calculate one-step-ahead predictions
...
calc.osa.resid <- function(inp, rep){
    ns <- length(time)
...
# Extract parameters from rep (sd report)
get.par <- function(lab, rep=rep, exp=FALSE, random=FALSE, fixed=FALSE){
    if(random){
        ind <- which(names(rep$par.random)==lab)
        ...
        if(fixed){
            ind <- which(names(rep$par.fixed)==lab)
            est <- rep$par.fixed[ind]
            sd <- sqrt(diag(rep$cov.fixed))[ind]
            ll <- est - 1.96*sd
            ul <- est + 1.96*sd
        } else {
            ind <- which(names(rep$value)==lab)
            est <- rep$value[ind]
            sd <- rep$sd[ind]
            ll <- est - 1.96*sd
            ul <- est + 1.96*sd
...
## Make a continuous line with arrow heads showing the direction between each data point
...
# Calculate time constant NEW (proportion of Binf reached)
...
plot.pmod <- function(inp, rep){
    ...
    Cscal <- 1
    Cpred <- osar$logCpred
    plot(inp$timeC, log(inp$obsC), typ='p', ylim=range(c(Cpred,log(inp$obsC),1.08*c(Cpred,log(inp$obsC)))), main=paste('Ljung-Box test p-value:',round(osar$logCpboxtest$p.value,3)), ylab=paste('Catch, Cscal:',Cscal), xlim=range(c(inp$timeC,inp$timeC[inp$nobsC]+1)))
    clr <- 'black'
    lines(osar$timeC, Cpred, col=clr)
    points(osar$timeC, Cpred, pch=20, cex=0.7, col=clr)
    legend('topright', 'One-step pred.', lty=1, col=clr, pch=20, pt.cex=0.7)

    # OSAR ACF
    acf(osar$logCpres, main='ACF of OSA residuals')
    ...
    main <- paste('alpha:',pl$alpha,'r:',round(rest[2],3),'q:',round(qest[2],3))
    par(mar=c(5,4,4,4))
    ...
    plot(inp$timeC, inp$obsC/Cscal, typ='n', main=paste('MSY:',round(MSY[2]/Cscal)), xlab='Time', ylab=paste('Catch'), xlim=range(c(inp$time, tail(inp$time,1)+1)), ylim=range(c(1.3*inp$obsC, Cpredest[,1:3], 0.8*inp$obsC, Cinfp[2], Cpmsy[2], Cp))/Cscal)
    ...
    points(tail(inp$timeC,1)+dtpred, Cp/Cscal, pch=21, bg='yellow')
    ...
    plot(Best[-1, 2]/Bmsy[2], Pest[, 2]/dt/Bmsy[2], typ='l', ylim=range(Pest[,2]/dt/Bmsy[2], Pst/Bmsy[2]), xlim=xlim, xlab='B/Bmsy', ylab='Production/Bmsy', col=4)
    lines(Bplot/Bmsy[2], Pst/Bmsy[2], col=1)
    #arrow.line(Best[-1, 2]/Bmsy[2], Pest[,2]/dt/Bmsy[2], length=0.05)
    abline(v=1, lty=3)
    ...
    tc <- tc.fun2(Fest[, 2], Kest[2], rest[2], sdbest[2], Best[, 2], p=0.99, lamperti=lamperti)
    k <- 6
    pvec <- (0:99)/100
    npvec <- length(pvec)
    tcfix <- tc.fun2(Fest[k, 2], Kest[2], rest[2], sdbest[2], Best[k, 2], pvec, lamperti=lamperti)
    Bcur <- Best[ns, 2]
    B0vec <- round(c(0.5*Bcur, 2*Bcur))
    nB0vec <- length(B0vec)
    tcmsy <- matrix(0, nB0vec, npvec)
    for(i in 1:nB0vec) tcmsy[i, ] <- tc.fun2(Fmsy[2], Kest[2], rest[2], sdbest[2], B0vec[i], pvec, lamperti=lamperti)
    tccur <- tc.fun2(Fmsy[2], Kest[2], rest[2], sdbest[2], Best[ns, 2], pvec, lamperti=lamperti)
    #tcmsy <- tc.fun2(Fmsy[2], Kest[2], rest[2], sdbest[2], Bmsy[2], pvec, lamperti=lamperti)
