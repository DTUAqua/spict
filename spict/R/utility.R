...
plotspict.osar <- function(rep, collapse.I=TRUE){
    if('osar' %in% names(rep)){
        ...
        fun <- function(time, res, add=FALSE, col=1, pch=1, ...){
            ...
            if(!add) plot(time, res, xlab='Time', typ='n', ...)
            lines(time, res, col='lightgray')
            if(min(inp$dtc)<1){
                ...
                numods <- length(umods)
                for(i in 1:numods){
                    inds <- which(mods == umods[i])
                    points(time[inds], res[inds], col=1, pch=20+pch, bg=cols[i])
                }
            } else {
        ...
        if(collapse.I){
            ...
            ylim <- range(na.omit(rep$osar$logIpres[[1]]))
...
plotspict.diagnostic <- function(rep){
    ...
    par(mfrow=mfrow)
    # Plot data
    plot.data.fun <- function(time, obs, pch=1, col=1, add=FALSE, ...){
        if(!add) plot(time, obs, xlab='Time', typ='n', ...)
        lines(time, obs, col='lightgray')
        if(min(inp$dtc)<1){
            cols <- rep(c(4, 3, 2, 5, 1, 6, 7, 'orange'), 10) # Make sure we have enough colors
            mods <- time%%1
            umods <- unique(mods)
            ...
            points(time, obs, col=col, pch=pch)
        }
        if(!add) box(lwd=1.5)
    }
    plot.data.fun(inp$timeC, log(inp$obsC), ylab='log catch data', main='Catch')
    #plot.data.fun(inp$timeI[[1]], inp$obsI[[1]])
    for(i in 1:inp$nindex) plot.data.fun(inp$timeI[[i]], log(inp$obsI[[i]]), ylab=paste('log index',i,'data'), main=paste('Index', i), pch=i)
    ...
    if('osar' %in% names(rep)){
        ...
        for(i in 1:inp$nindex){
            ...
            colmain <- ifelse(pval<0.05, 'red', 'forestgreen')
            qqnorm(resI[[i]], main=paste0('I', i, ', Shapiro p-val: ', pval), col.main=colmain, pch=i)
            abline(0,1)
            box(lwd=1.5)
        }
    } else {
...
plotspict.f <- function(rep, logax=FALSE, main=-1, plot.legend=TRUE, ylim=NULL){
    if(!'sderr' %in% names(rep)){
        ...
        if(flag){
            ...
            if(length(ylim)!=2) ylim <- range(c(Ff, Fmsy[1:3], tail(Fest[, 2],1)))
        } else {
...
plotspict.ffmsy <- function(rep, logax=FALSE, main=-1, plot.legend=TRUE, ylim=NULL){
    if(!'sderr' %in% names(rep)){
        ...
        if(flag){
            fininds <- which(is.finite(Ff))
            if(length(ylim)!=2) ylim <- range(c(Ff[fininds], 0.95*Fmsy[1]/Fmsy[2], 1.05*Fmsy[3]/Fmsy[2]))
        } else {
            fininds <- which(apply(cbind(cl, cu), 1, function(x) all(is.finite(x))))
...
plotspict.fb <- function(rep, logax=FALSE, plot.legend=TRUE, ext=TRUE, rel.axes=FALSE){
    if(!'sderr' %in% names(rep)){
        ...
        if(min(inp$dtc) < 1){
            ...
            lines(exp(alb$annvec)/bscal, exp(alf$annvec)/fscal, col=maincol, lwd=1.5)
...
plotspict.catch <- function(rep, main=-1, plot.legend=TRUE, ylim=NULL){
    if(!'sderr' %in% names(rep)){
        ...
        if(length(ylim)!=2) ylim <- range(c(clf[fininds], cuf[fininds], 0.9*MSY[1], 1.07*MSY[3]))/Cscal
