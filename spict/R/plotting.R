...
#'
#' @export
plot2 <- function(x, stamp=get.version(), bar=TRUE, ...){
    check.rep(x)
    rep <- x
    logax <- FALSE # Take log of relevant axes? default: FALSE
    inp <- rep$inp
    if (inp$reportall){
        ...
        # Catch
        if(bar){
            plotspict.catch.bar(rep, qlegend=FALSE, stamp='')
        }else{
            plotspict.catch(rep, qlegend=FALSE, stamp='')
        }
        # F versus B
...
#'
#' @export
plotspict.catch.bar <- function(rep, main='Catch', ylim=NULL, qlegend=TRUE, lcol='blue',
                            xlab='Time', ylab=NULL, stamp=get.version()){
    check.rep(rep)
    if (!'sderr' %in% names(rep)){
        manflag <- any(names(rep) == "man")
        ...
        if (min(inp$dtc) < 1){
            ...
            inds <- which(!is.na(al2$annvec))
            time <- al2$anntime[inds]
            dtcc <- annual(inp$timeCpred[indest], dtc[indest],sum)$annvec[inds]
            c <- al2$annvec[inds]
            cl <- al1$annvec[inds]
            cu <- al3$annvec[inds]
            al1p <- annual(inp$timeCpred[indpred], Cpredest[indpred, 1]/dtc[indpred])
        ...
        if (!ylimflag){
            if (length(ylim) != 2){
                ylim <- range(c(0, 1.07*cuf[fininds], 1.07*max(MSY[, 3])), na.rm=TRUE)/Cscal
                if (inp$dtpredc > 0){
                    ylim <- range(ylim, clf[fininds], cuf[fininds])
                }
            }
        ...
        polygon(c(repmax$inp$time, rev(repmax$inp$time)),
                c(MSYvec$ll,rev(MSYvec$ul)), col=cicol, border=cicol)
        if(!manflag) abline(v=tail(inp$timeC + inp$dtc,1), col='gray')
        obsc <- obs/Cscal
        xplot <- c(timeo[1],rep(timeo[-1],each=2),max(timeo)+inp$dtpredc)
        yplot <- rep(obsc,each=2)
        nobs <- length(timeo[-1])
        barcol <- rgb(t(col2rgb("chocolate4")/255),alpha=0.2)
        polygon(c(range(xplot),rev(xplot)),
                c(rep(0,2),rev(yplot)),lwd=1.2,col=barcol,border="black")
        ## no dobule drawn lines
        diffobs <- diff(obsc)
        y1s <- rep(NA,nobs)
        for(i in 1:nobs) y1s[i] = ifelse(obsc[i] < obsc[i+1], obsc[i], obsc[i+1])
        segments(x0=timeo[-1], y0=rep(0,nobs), x1=timeo[-1], y1=y1s,lwd=1.2)
        lines(repmax$inp$time, MSYvec$msy)
        points(time+dtcc/2, c, col=lcol, lwd=1.5, cex=1.1,pch=16)
        lines(time+dtcc/2, c, col=lcol, lwd=1.5,lty=2)
        xl <- (time+dtcc/2)[length(time)]
        cicol2 <- rgb(0, 0, 1, 0.1)
        arrows(time+dtcc/2, cl, time+dtcc/2, cu, length=0.05, angle=90, code=3, col=lcol,lwd=1.5)
        # Highlight influential index observations
        if ('infl' %in% names(rep) & min(inp$dtc) == 1){
            infl <- rep$infl$infl[1:inp$nobsC, ]
            cols <- apply(!is.na(infl), 1, sum)
            ncols <- length(unique(cols))
            inds <- which(cols>0)
            points(inp$timeC[inds]+inp$dtcc[inds]/2, inp$obsC[inds]/Cscal, pch=21, cex=0.9, bg=cols[inds])
        }
        if ('true' %in% names(inp)){
            abline(h=inp$true$MSY, col=true.col(), lty=1)
            abline(h=inp$true$MSY, col='black', lty=3)
        ...
        if(manflag){
            ...
            for (i in 1:nman){
                repi <- rep$man[[ scenarios[i] ]]
                inpi <- repi$inp
                esti <- get.par("logCpred", repi, exp=TRUE)[,2]
                time <- repi$inp$timeCpred
                ## check for multi-annual data points
                dtc <- c(diff(time),inpi$dtpredc)
                ## split into annual data points
                ...
                x <- time[maninds] + inpi$dtpredc/2
                points(x, esti2[maninds], col=man.cols()[i], lwd=1.5,pch=16)
                ## int period
                lastobs <- inpi$timerangeObs[2]
                if((manstart-lastobs) > 0){
                    indlastobs <- which(time >= lastobs & time < manstart)
                    intinds <- indlastobs
##                    intinds <- (indlastobs[1] - index.shift):maninds[1]
                    xint <- time[intinds] + dtc[intinds]/2
                    points(xint, esti2[intinds], col=man.cols()[i], lwd=1.5)
                    xintl <- c(xl,xint,x)
                    lines(xintl, esti2[c(max(indest),intinds,maninds)],
                          col=man.cols()[i], lwd=1.5, lty=3)
                }else{
                    xintl <- c(xl,x)
                    lines(xintl, esti2[c(max(indest),maninds)],
                          col=man.cols()[i], lwd=1.5, lty=3)
                }
            }
            ##            add.manlines(rep, 'logCpred', index.shift=1, plot.legend=qlegend)
            ...
            if (inp$dtpredc > 0){
                points(timep+dtcp/2, cp, col=lcol, lty=3,pch=16)
                xintl <- c(xl,timep+dtcp/2)
                lines(xintl, c(c[max(indest)],cp), col=lcol, lty=3)
                arrows(timep+dtcp/2, clp, timep+dtcp/2, cup, length=0.05, angle=90,
                       code=0, col=lcol,lwd=1.5,lty=3)
                arrows(timep+dtcp/2, clp-0.1, timep+dtcp/2, clp, length=0.05, angle=90,
                       code=1, col=lcol,lwd=1.5,lty=1)
                arrows(timep+dtcp/2, cup, timep+dtcp/2, cup+0.1, length=0.05, angle=90,
                       code=2, col=lcol,lwd=1.5,lty=1)
            }
        }
        if (rep$opt$convergence != 0){
            warning.stamp()
