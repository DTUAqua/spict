...
#' @param plot.legend logical, if TRUE (default) adds legend to the plot
...
#' 
#' @param string Character string to stamp. If it is an empty string, NULL, or NA the function does nothing
#' @param cex Stamp cex.
...
ellipse <- function(x, scale, center, npoints) {
  x <- min(max(x, -1), 1)
  d <- acos(x)
  a <- seq(0, 2 * pi, length.out = npoints)
  t <- sqrt(qchisq(0.95, 2))
  data.frame(x = t * scale[1] * cos(a + d/2) + center[1],
             y = t * scale[2] * sin(a + d/2) + center[2])
}
...
make.ellipse <- function(inds, rep){
  ...
  ellipse(corBF[1,2], scale=sqrt(diag(covBF)), center=parBF, npoints=300)
}
...
#' @description Plots observed versus predicted catches.
#' @param rep A result report as generated by running fit.spict.
...
#' 
#' @description Plots estimated fishing mortality with Fmsy and associated confidence interval.
#' @param rep A result report as generated by running fit.spict.
#' @param logax Take log of y-axis? default: FALSE
#' @param main Title of plot.
#' @param ylim Limits for y-axis.
