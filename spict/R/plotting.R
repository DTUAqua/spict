...
#' @export
plotspict.biomass <- function(rep, stock=1, logax=FALSE, main='Absolute biomass', ylim=NULL,
    ...
    if (!'sderr' %in% names(rep)){
        ...
        scal <- 1
        cicol <- 'lightgray'
        if (inp$nindex[stock] == 0){
            nindexseq <- numeric(0)
            obsI <- NULL
        } else {
            nindexseq <- 1:inp$nindex[stock]
            obsI <- list()
        }
        for (i in nindexseq){
            obsI[[i]] <- inp$obsI[[stock]][[i]] / qest[inp$mapq[[stock]][i], 2]
        }
        # Use CV to check for large uncertainties
        fininds <- which(Best[, 5] < 5) 
        # Use CV to check for large uncertainties
        BBfininds <- unname(which(is.finite(BB[, 1]) & is.finite(BB[, 3]))) 
        if (length(ylim) != 2){
        ...
        plot(time, Best[, 2]/scal, typ='n', xlab=xlab, ylab=ylab, main=main, ylim=ylim,
             xlim=range(c(inp$time, time, tail(time, 1) + 0.5)), log=log)
        if (rel.axes){
        ...
        if (all(is.finite(unlist(Bmsyvec)))){
            polygon(c(time, rev(time)), c(Bmsyvec$ll,rev(Bmsyvec$ul)),
                    col=cicol, border=cicol)
        }
        cicol2 <- rgb(0, 0, 1, 0.1)
        if (!'yearsepgrowth' %in% names(inp) & rel.ci){
            polygon(c(time[BBfininds], rev(time[BBfininds])),
        ...
        if (plot.obs){
            for (i in nindexseq){
                #plot.col(timeI[[i]], inp$obsI[[i]]/qest[inp$mapq[i], 2], pch=i,
                #         do.line=FALSE, cex=0.6, add=TRUE, add.legend=FALSE)
                plot.col(inp$timeI[[stock]][[i]], obsI[[i]], pch=i,
                         do.line=FALSE, cex=0.6, add=TRUE, add.legend=FALSE)
            }
            if (qlegend){
                subyears <- unique(unlist(inp$timeI[[stock]]) %% 1)
                # Only plot legend
                plot.col(subyears, numeric(length(subyears)), typ='n', add=TRUE, add.legend=TRUE) 
            }
        ...
        lines(time, Bmsyvec$msy, col='black')
        # B CI
        lines(inp$time[indest], Bestfull[indest, 1]/scal, col=4, lty=2, lwd=1.5)
        lines(inp$time[indest], Bestfull[indest, 3]/scal, col=4, lty=2, lwd=1.5)
        lines(inp$time[indpred], Bestfull[indpred, 1]/scal, col=4, lty=2)
        lines(inp$time[indpred], Bestfull[indpred, 3]/scal, col=4, lty=2)
        #}
        # B/Bmsy CI
        cicol3 <- rgb(0, 0, 1, 0.2)
        if (!'yearsepgrowth' %in% names(inp)){
            lines(inp$time[indest], BBfull[indest, 1]/scal*Bmsy[2], col=cicol3, lty=1, lwd=1)
            lines(inp$time[indest], BBfull[indest, 3]/scal*Bmsy[2], col=cicol3, lty=1, lwd=1)
            lines(inp$time[indpred], BBfull[indpred, 1]/scal*Bmsy[2],
                  col=cicol3, lty=1, lwd=1)
            lines(inp$time[indpred], BBfull[indpred, 3]/scal*Bmsy[2],
                  col=cicol3, lty=1, lwd=1)
        }
        if ('yearsepgrowth' %in% names(inp)){
...
#' plotspict.bbmsy(rep)
#' @export
plotspict.bbmsy <- function(rep, stock=1, logax=FALSE, main='Relative biomass', ylim=NULL,
    ...
    if (!'sderr' %in% names(rep)){
        ...
        inp <- rep$inp
        # Biomass plot
        Kest <- get.par('logK', rep, exp=TRUE, fixed=TRUE)[stock, ]
        Bmsy <- get.par('logBmsy', rep, exp=TRUE)[stock, ]
        Bmsyvecfull <- get.msyvec(inp, Bmsy)
        if (!all(is.na(Bmsyvecfull$msy))){ # Don't plot if all are NA
            ...
            if (inp$nindex[stock] == 0){
                ...
                obsI <- list()
                for (i in nindexseq){
                    obsI[[i]] <- inp$obsI[[stock]][[i]]/qest[inp$mapq[[stock]][i], 2]/Bmsy[2]
                }
            }
            ...
            plot(time, BB[,2], typ='n', xlab=xlab, ylab=expression(B[t]/B[MSY]),
            ...
            if (plot.obs){
                for (i in nindexseq){
                    plot.col(inp$timeI[[stock]][[i]], obsI[[i]], pch=i, do.line=FALSE, cex=0.6,
            ...
            cicol3 <- rgb(0, 0, 1, 0.2)
            lines(inp$time[indest], BBfull[indest, 1], col=cicol3, lty=1, lwd=1)
            lines(inp$time[indest], BBfull[indest, 3], col=cicol3, lty=1, lwd=1)
            lines(inp$time[indpred], BBfull[indpred, 1], col=cicol3, lty=1, lwd=1)
            lines(inp$time[indpred], BBfull[indpred, 3], col=cicol3, lty=1, lwd=1)
            abline(h=lineat)
            if (rep$opt$convergence != 0){
                warning.stamp()
            }
...
#' @return Nothing.
#' @export
plotspict.growth <- function(rep, stock=1, logax=FALSE, main='Time-varying growth', ylim=NULL,
                            xlim=NULL, xlab='Time', plot.ci=TRUE, stamp=get.version()){
    if (!'sderr' %in% names(rep) & rep$inp$timevaryinggrowth){
        log <- ifelse(logax, 'y', '')
        inp <- rep$inp
        rre <- get.par('logrre', rep, exp=TRUE)[inp$idstock == stock, ]
...
#' plotspict.f(rep)
#' @export
plotspict.f <- function(rep, stock=1, logax=FALSE, main='Absolute fishing mortality', ylim=NULL,
    ...
    if (!'sderr' %in% names(rep)){
        ...
        cicol <- 'lightgray'
        # Get parameters
        FFstock <- get.par('logFstockFmsy', rep, exp=TRUE)[inp$idstock == stock, ]
        ...
        if (quarterly){ # Plot estimated sub annual F 
            lines(inp$time, Fstock[, 2], col=rgb(0, 0, 1, 0.4))
        }
        abline(v=inp$time[inp$indlastobs], col='gray')
        if (plot.obs){
...
#' plotspict.ffmsy(rep)
#' @export
plotspict.ffmsy <- function(rep, stock=1, logax=FALSE, main='Relative fishing mortality', ylim=NULL,
                            plot.obs=TRUE, qlegend=TRUE, lineat=1, xlab='Time',
                            stamp=get.version()){
    if (!'sderr' %in% names(rep)){
        log <- ifelse(logax, 'y', '')
        ...
        cicol <- 'lightgray'
        qf <- get.par('logqf', rep, exp=TRUE)
        #Fest <- get.par('logFstock', rep, exp=TRUE)
        FF <- get.par('logFstockFmsy', rep, exp=TRUE)[inp$idstock == stock, ]
        #logFF <- get.par('logFFmsy', rep)[inp$idstock == stock, ]
        #Fmsy <- get.par('logFmsy', rep, exp=TRUE)
        #Fmsyvec <- get.msyvec(inp, Fmsy)
        # Get relevant indices
        indest <- intersect(inp$indest, inp$indsstock[[stock]])
        ...
        if (quarterly){
            ...
            cup <- FF[indpred, 3]
            timef <- inp$time[inp$indsstock[[stock]]]
            clf <- FF[inp$indsstock[[stock]], 1]
            Ff <- FF[inp$indsstock[[stock]], 2]
            cuf <- FF[inp$indsstock[[stock]], 3]
        }
        flag <- length(cu) == 0 | all(!is.finite(cu))
        if (flag){
            fininds <- which(is.finite(Ff))
        ...
        abline(v=inp$time[inp$indlastobs], col='gray')
        if (plot.obs){
            #plot.col(inp$timeE, inp$obsE/inp$dte*qf[2]/Fmsy[2], cex=0.7, do.line=FALSE,
...
#' plotspict.fb(rep)
#' @export
plotspict.fb <- function(rep, stock=1, logax=FALSE, plot.legend=TRUE, ext=TRUE, rel.axes=FALSE,
    ...
    if (!'sderr' %in% names(rep)){
        ...
        logBest <- get.par('logB', rep)[inp$idstock == stock, ]
        Fest <- get.par('logFstock', rep, exp=TRUE)[inp$idstock == stock, ]
        logFest <- get.par('logFstock', rep)[inp$idstock == stock, ]
        ns <- dim(Best)[1]
        Fp <- Fest[ns, ]
        inds <- c(which(names(rep$value) == 'logBmsy')[stock],
                  which(names(rep$value) == 'logFmsy')[stock])
        ...
        if (quarterly){ # Quarterly
            alb <- annual(inp$time, logBest[inp$indsstock[[stock]], 2])
            alf <- annual(inp$time, logFest[inp$indsstock[[stock]], 2])
            ...
            if (inp$timevaryinggrowth){
                bbb <- get.par('logBBmsy', rep, exp=TRUE)[indest, 2]
                fff <- get.par('logFFmsy', rep, exp=TRUE)[indest, 2]
            } else {
                bbb <- Best[indest, 2]/bscal
                fff <- Fest[indest, 2]/fscal
            }
            pind <- rep$inp$dtprediind
            fbtime <- inp$time[indest]
        }
        ...
        plot(Bmsy[2]/bscal, Fmsy[2]/fscal, typ='n', xlim=xlim, xlab=xlab,
             ylab=ylab, ylim=ylim, log=log)
        if (ext){
            if (logax){
...
#' @export
plotspict.production <- function(rep, stock=1, n.plotyears=40, main='Production curve',
                                 stamp=get.version()){
    if (!'sderr' %in% names(rep)){
        inp <- rep$inp
        finds <- which(inp$targetmap[, 1] == stock)
...
plot.spictcls <- function(x, ...){
    ...
    if ('par.fixed' %in% names(rep) & rep$inp$do.sd.report){
        ...
        if (inp$reportall){
            ...
            # F versus B
            plotspict.fb(rep, stock=stock, rel.axes=rep$inp$timevaryinggrowth,
                         logax=logax, plot.legend=TRUE, stamp='')
        } else {
