...
#' @name fitrobust.spict
#' @title Fit a robust version of spict to data.
#' @details Fits the model using the TMB package and returns a result
#'     report containing estimates of model parameters, random effects
#'     (biomass and fishing mortality), reference points (Fmsy, Bmsy,
#'     MSY) including uncertainties given as standard deviations.
#'
#' Model parameters using the formulation of Fletcher (1978):
#' \itemize{
#'   \item{"logn"}{ Parameter determining the shape of the production curve as in the generalised form of Pella & Tomlinson (1969).}
...
#'
#' @param inp List of input variables as output by check.inp.
#' @param lonpriors List of prior distributions (see check.inp for
#'     format of priors) for logn parameter. Default is a normal
#'     distribution with mean = 2 and sd decreasing from 1 to 0.01.
#' @return A result report containing estimates of model parameters,
#'     random effects (biomass and fishing mortality), reference
#'     points (Fmsy, Bmsy, MSY) including uncertainties given as
#'     standard deviations.
#' @export
#' @examples
#' data(pol)
#' rep <- fitrobust.spict(pol$albacore)
#' Bmsy <- get.par('logBmsy', rep, exp=TRUE)
#' summary(rep)
#' plot(rep)
#' @import TMB
fitrobust.spict <- function(inp,
                            lognpriors=list(c(log(2), 1, 1),
                                           c(log(2), 0.5, 1),
                                           c(log(2), 0.25, 1),
                                           c(log(2), 0.01, 1))){
    ## default settings first
    fit<-suppressWarnings(fit.spict(inp))
    i<-1
    while(fit$opt$convergence!=0 && i<=length(lognpriors)){
        cat("Refitting with logn prior std.dev ",
            lognpriors[[i]][2]," (",i,"/",length(lognpriors),") \n")
        inp$priors$logn <- lognpriors[[i]]
        fit<-suppressWarnings(fit.spict(inp))
        i<-i+1
    }    
