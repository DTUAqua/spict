...
#' @param include.abs Include absolute B and F in management summary.
...
#'
#' The combination of the arguments "fractiles", "breakpointB", and "safeguardB"
#' allow the specification of a number of different harvest control rules:
#'
#'\itemize{
#'
#' \item{MSY hockey-stick rule: Fishing at F_{MSY} above a certain biomass
#'     reference level (here defined as a fraction of B_{MSY} with
#'     \code{breakpointB}). Below the reference level, fishing is reduced
#'     linearly to 0 as suggested in ICES (2017).}
#'
...
#'     biomass level (e.g. 0.3 B_{MSY}, defined by \code{safeguardB$limitB}) is
#'     smaller or equal to a specified risk aversion probability (e.g. 95\%,
#'     defined by \code{safeguardB$prob}), fishing at F_{MSY} or following the
#'     hockey-stick rule (if \code{breakpoint != 0}), otherwise reduce fishing
#'     mortality to meet specified risk aversion probability
#'     (\code{safeguardB$prob}) as introduced in ICES (2018).}
#'
#' \item{A MSY hockey-stick rule that accounts of uncertainty of estimated
#'     quantities: Fishing at 35th percentile of F_{MSY} above the 35th
#'     percentile of 0.5 \eqn{B/B_{MSY}} (\code{breakpointB = 0.5}) and 35th
#'     percentile of linearly reduced F_{MSY} below the 35th percentile of 0.5
...
#'     ffmsy=0.35)}, \code{breakpointB = 0.5} as introduced in ICES (2019).}
#'
#' }
#'
#' \subsection{Fractiles}{ By default, the median (fractile of 0.5) is used for
#' the stock status (\eqn{B/B_{MSY}}, \eqn{F/F_{MSY}}) and predicted catch
#' distribution. A more precautionary approach is to used fractiles lower than
#' the median (0.5) to account for the estimated uncertainty. The arguments of
#' the `fractiles` are:
#'
#' \itemize{
