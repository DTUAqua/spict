...
#' @param include.abs Include absolute values of management quantities.
#' @param include.int Include management quantities during the intermediate year.
...
sumspict.manage <- function(rep, include.EBinf=FALSE,
    ...
    ## intermediate year
    Cintyear <- vector("list", nsc)
    Bintyear <- matrix(0, nsc, 3)
    colnames(Bintyear) <- get.cn(Bn)
    Fintyear <- matrix(0, nsc, 3)
    colnames(Fintyear) <- get.cn(Fn)
    BBintyear <- matrix(0, nsc, 3)
    colnames(BBintyear) <- get.cn(BBn)
    FFintyear <- matrix(0, nsc, 3)
    colnames(FFintyear) <- get.cn(FFn)
    perc.dBint <- numeric(nsc)
    perc.dFint <- numeric(nsc)
    intFlag <- rep(NA, nsc)
    ...
    for(i in 1:nsc){
        ...
        indnextC <- which(rp$inp$timeCpred == min(rp$inp$maninterval))
        ...
        ## intermediate year
        timeC <- max(format(round(c(rp$inp$timerange[1],rp$inp$lastCatchObs),2),nsmall=2))
        timeM <- min(format(round(range(rp$inp$maninterval),2),nsmall=2))
        intFlag[i] <- ifelse(timeC == timeM, FALSE, TRUE)
        if(intFlag[i]){
            perc.dBint[i] <- get.pdelta('logB',rp, int = TRUE)
            perc.dFint[i] <- get.pdelta('logF',rp, int = TRUE)
            ## using end of observed catches as C evaltime for intermediate year
            indintC <- which(rp$inp$timeCpred >= as.numeric(timeC) & rp$inp$timeCpred < min(rp$inp$maninterval))
            ## using start of maninterval as time evaltime for intermediate year
            indint <- which(rp$inp$time == min(rp$inp$maninterval))
            indintF <- which(rp$inp$time == min(rp$inp$maninterval)) - 1
            Cintyear[[i]] <- round(get.par('logCpred', rp, exp=TRUE)[indintC, 1:3], 1)
            Bintyear[i, ] <- round(get.par('logB', rp, exp=TRUE)[indint, 1:3], 1)
            Fintyear[i, ] <- round(get.par('logF', rp, exp=TRUE)[indintF, 1:3], 2)
            BBintyear[i, ] <- round(get.par('logBBmsy', rp, exp=TRUE)[indint, 1:3], 2)
            FFintyear[i, ] <- round(get.par('logFFmsy', rp, exp=TRUE)[indintF, 1:3], 2)
        }
    }
    if(nsc == 1){
        cflag <- ""
    ...
    if(any(intFlag)){
        inds <- c(1,3)
        if(!inherits(Cintyear[[1]], "matrix")){
            dfint <- cbind("C" = do.call(rbind,Cintyear)[,2])
        }else{
            dfint <- cbind("C" = sapply(Cintyear,function(x) x[1,2]))
            for(i in 2:nrow(Cintyear[[1]])){
                dfint <- cbind(dfint, sapply(Cintyear,function(x) x[i,2]))
            }
        }
        dfint <- cbind(dfint, "B/Bmsy" = BBintyear[,2], "F/Fmsy" = FFintyear[,2])
        indintC <- format(rep$inp$timeCpred[which(rep$inp$timeCpred >=
                                                  max(c(rep$inp$timerange[1],rep$inp$lastCatchObs)) &
                                                  rep$inp$timeCpred <= min(rep$inp$maninterval))],nsmall=2)
        if(inherits(Cintyear[[1]], "matrix")){
            lab <- rep(NA,length(indintC)-1)
            for(i in 1:(length(indintC)-1)) lab[i] <- paste(c(indintC[i],indintC[i+1]), collapse="-")
            colnames(dfint) <- c(paste0("C[",lab,"]"),"B/Bmsy","F/Fmsy")
        }
        dfintunc <- cbind(BBintyear[,inds,drop=FALSE], FFintyear[,inds,drop=FALSE])
        if(include.abs){
            dfint <- cbind(dfint,
                           "B" = Bintyear[,2], "F" = Fintyear[,2],
                           "perc.dB" = perc.dBint, "perc.dF" = perc.dFint)
            dfintunc <- cbind(dfintunc,
                              Bintyear[,inds,drop=FALSE], Fintyear[,inds,drop=FALSE])
        }
        qinds <- grep('q', colnames(dfint))
        colnames(dfint)[qinds] <- sub('q', '/', colnames(dfint)[qinds]) # Replace q with /
        qinds <- grep('q', colnames(dfintunc))
        colnames(dfintunc)[qinds] <- sub('q', '/', colnames(dfintunc)[qinds]) # Replace q with /
        rownames(dfint) <- rn
        rownames(dfintunc) <- rn
    }

    ## Start printing stuff
    ## Warning when include.int=TRUE but no intermediate year
    if(include.int && all(!intFlag)) cat("Cannot include information about intermediate period, because management period starts right after last observations. Please check the setting of 'inp$maninterval'.\n\n")
    ...
    cat('\nPredicted catch for the management period and \nstates at the management evaluation time:\n\n')
    print(df)
    cat('........................................\n')
    if (include.unc){
        cat('\n95% confidence intervals for the states:\n\n')
        print(dfunc)
        cat('........................................\n')
    }
    ## intermediate year
    if (include.int && any(intFlag)){
        cat(paste0('\nPredicted catch for the intermediate period (',paste(indintC,collapse="-"),
                   ') and \nstates at end of the intermediate period (',
                  rep$inp$time[which(rep$inp$time == min(rep$inp$maninterval))],'):\n\n'))
        print(dfint)
        cat('........................................\n')
        if (include.unc){
            cat(paste0('\n95% confidence intervals for the states during the intermediate period (',
                       paste(indintC,collapse="-"),
                       '):\n\n'))
            print(dfintunc)
            cat('........................................\n')
        }
    ...
    if (include.int && any(intFlag)) res[["int"]] <- dfint
    if (include.unc && include.int && any(intFlag)) res[["intunc"]] <- dfintunc
