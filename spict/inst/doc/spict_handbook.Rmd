...
vignette: >
  %\VignetteIndexEntry{SPiCT Manual}
...
part of the usual model fitting with SPiCT. The default settings can be altered
...
the catch of 2021 is of interest, then `maninterval` is set to represent the
start and end of the forecast interval: `c(2021,2022)`. In addition to the
forecast interval a fishing scenario can to be specified. This is done by
specifying a factor (`ffac`) to multiply the fishing mortality at the start of
...
The argument `scenarios` allows you to limit the number of scenarios to be run
by indicating scenario numbers, e.g. `scenarios = c(1,5,2,8)`, or scenario
names, e.g. `scenarios = c("noF", "ices", "incrF25")`. The list of all possible
management scenarios included in the `manage` function with index and name are:
...
\begin{enumerate}
   ...
   \item \textbf{redF25}: Reduce F by 25\%.
   \item \textbf{incrF25}: Increase F by 25\%,
...
If any plotting function of the `plotspict.` family is called on SPiCT object
...
the start and end of the management interval. In order to go back to the
previous plots, one can remove the management scenarios (see below) or assign
the results of the `manage` function to a different object, e.g. `man <-
...
in-year advice @wklifeix). For example, applying one of the default scenarios
for a half year period starting in the middle of 1990:

first in year advice example
...
After comparing the implications of different management scenarios, it might of
interest to extract an individual scenario for further analyses, plotting, or
reporting. Individual scenarios can be extracted from the SPiCT object by
referencing their names or positioning in the list `rep$man`. The extracted
object is a standard SPiCT object of the class `spictcls`, which allows for
example to plot trajectories and uncertainties of predicted quantities by:
