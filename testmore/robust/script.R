...
sink("res.out")
parest1 <- sumspict.parest(fit)
## Round m and K to the closest integer
idx <- rownames(parest1) %in% c("m", "K")
parest1[idx, 1:3] <- round(parest1[idx, 1:3])
parest2 <- sumspict.parest(fit2)
parest2[idx, 1:3] <- round(parest2[idx, 1:3])
print(round(parest1, dec.val))
print(round(parest2, dec.val))
sink()
